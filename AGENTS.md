...
9. Never increase Android `versionCode` to force-install an older source tree.
10. Debug APK generation and installation are disabled because debug Compose
    builds have unusable scroll performance. Use `compileDebugKotlin` only for
    compile validation.
11. Use `npm run android:release` for the R8 release APK. Use only the printed
    `android/app/build/outputs/apk/release/app-release.apk` path.
12. Before merge, use `npm run android:test-release` for a local R8 test APK;
    it is never a production release and remains distinct from
    `npm run android:release`.
13. Use `npm run android:verify -- <apk>` before handling an APK and
