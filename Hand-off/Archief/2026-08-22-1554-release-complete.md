...
- Physical-device test passed: the R8 APK installed with `adb install -r`, the
  vault was reselected to renew SAF write access, and the app now works on the
  phone. Manual APK installation from `Download` is not the supported test path.
